...
import uuid
from utils.decimal_helpers import safe_sum, safe_float  # CRITICAL FIX (Mar 9, 2026): Handle Decimal128 in sum operations
        ...
        try:
            ...
            for transaction in transactions:
                if transaction['type'] == 'purchase':
                    total_owed += safe_float(transaction['amount'])
                elif transaction['type'] == 'payment':
                    paid_amount += safe_float(transaction['amount'])
            ...
            remaining_owed = round(total_owed - paid_amount, 2)
