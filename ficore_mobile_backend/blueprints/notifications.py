...
def init_notifications_blueprint(mongo, token_required, serialize_doc):
    ...
    notifications_bp.mongo = mongo
    notifications_bp.serialize_doc = serialize_doc
...
def get_user_notifications(current_user):
    ...
    try:
        ...
        # Get total count
        total_count = notifications_bp.mongo.db.user_notifications.count_documents(query)
        
        # Get notifications with pagination
        skip = (page - 1) * limit
        notifications = list(notifications_bp.mongo.db.user_notifications.find(query)
                           .sort('timestamp', -1)  # Newest first
                           .skip(skip)
                           .limit(limit))
        
        # Get unread count
        unread_count = notifications_bp.mongo.db.user_notifications.count_documents({
        ...
        return jsonify({
            'success': True,
            'data': {
                'notifications': [notifications_bp.serialize_doc(n) for n in notifications],
                'pagination': {
                    'page': page,
                    'limit': limit,
                    'total': total_count,
...
def mark_notifications_read(current_user):
    ...
    try:
        ...
        if mark_all:
            ...
            result = notifications_bp.mongo.db.user_notifications.update_many(
                {
                    'userId': ObjectId(user_id),
                    'isRead': False,
                    'isArchived': False
