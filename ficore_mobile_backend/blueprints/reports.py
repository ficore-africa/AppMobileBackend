...
def filter_by_date_range(items, date_field, start_date, end_date):
    ...
    def refund_corrupted_report(current_user, job_id):
        """Refund credits for corrupted PDF reports"""
        try:
            # Find the credit transaction for this job
            credit_transaction = mongo.db.credit_transactions.find_one({
                'userId': current_user['_id'],
            ...
            if not credit_transaction:
                return jsonify({
                    'success': False,
                    'message': 'No credit transaction found for this job'
                }), 404
            
            # Check if already refunded
            existing_refund = mongo.db.credit_transactions.find_one({
                'userId': current_user['_id'],
                'metadata.originalTransactionId': credit_transaction['_id'],
                'type': 'refund'
            })
            ...
            if existing_refund:
                return jsonify({
                    'success': False,
                    'message': 'Refund already processed for this report'
                }), 400
            
            # Validate that PDF is actually corrupted
            bg_generator = get_background_generator(mongo.db)
            job = bg_generator.get_job(job_id)
            
            if job and job.get('file_id'):
                file_buffer = bg_generator.get_file(job['file_id'])
                if file_buffer and _validate_pdf_integrity(file_buffer):
                    return jsonify({
                        'success': False,
                        'message': 'PDF is valid, refund not applicable'
                    }), 400
            
            # Process refund
            refund_amount = abs(credit_transaction['amount'])  # Make positive
            ...
            refund_transaction = {
                ...
                'description': f'Refund for corrupted report - {credit_transaction["metadata"]["reportType"].upper()}',
                'balanceBefore': current_balance,
                'balanceAfter': new_balance,
                'status': 'completed',
                'metadata': {
                    'reportType': credit_transaction['metadata']['reportType'],
                    'reportJobId': job_id,
                    'originalTransactionId': credit_transaction['_id'],
