...
"""
    ...
    def generate_financial_report(self, user_data, export_data, data_type='all', tag_filter="all"):
        ...
        # Bridge Line - Strategic messaging
        bridge_line = Paragraph(
            '<i>Helping you grow your business, one record at a time.</i>',
            self.styles['InfoText']
        )
        story.append(bridge_line)
        story.append(Spacer(1, 12))
    ...
    def generate_statement_of_affairs(self, user_data, financial_data, tax_data, assets_data, 
        ...
        bridge_line = Paragraph("Helping you grow your business, one record at a time.", ParagraphStyle(
            'BridgeLine',
...
"""
        ...
        story.append(Paragraph("Detailed Financial Schedules", self.styles['CustomTitle']))
        story.append(Spacer(1, 12))
        ...
        # Create detailed income table
        income_detail_data = [
            ['Category', 'Source Type', 'Amount (N)', 'Description', 'Date']
        ]
        
        total_income_detailed = 0
        for category, items in income_breakdown.items():
            # Category header
            income_detail_data.append([f"INCOME: {category.upper()}", '', '', '', ''])
            
            category_total = 0
            for item in items:
                amount = item['amount']
                category_total += amount
                total_income_detailed += amount
            ...
            income_detail_data.append(['', '', '', '', ''])  # Spacer
        
        # Total income
        income_detail_data.append([
            'TOTAL INCOME',
            '',
            format_currency(total_income_detailed),
            f'{len(incomes)} transactions',
            ''
        ])
        ...
        # Add expenses to the same table
        income_detail_data.append(['', '', '', '', ''])  # Spacer
        
        total_expenses_detailed = 0
        for category, items in expense_breakdown.items():
            # Category header
            income_detail_data.append([f"EXPENSE: {category.upper()}", '', '', '', ''])
            
            category_total = 0
            for item in items:
                amount = item['amount']
                category_total += amount
                total_expenses_detailed += amount
        ...
        income_detail_data.append([
            'TOTAL EXPENSES',
            '',
            format_currency(total_expenses_detailed),
            f'{len(expenses)} transactions',
            ''
        ])
        
        # Net profit calculation
        net_profit_detailed = total_income_detailed - total_expenses_detailed
        income_detail_data.append(['', '', '', '', ''])  # Spacer
        income_detail_data.append([
            'NET PROFIT/(LOSS)',
            '',
            format_currency(net_profit_detailed),
            'Income - Expenses',
            ''
        ])
        ...
        pnl_detail_style = TableStyle([
            ...
            ('FONTNAME', (0, 1), (0, -1), 'Helvetica-Bold'),  # Category names
            # Highlight totals
        ...
        # Schedule 2: Detailed Cash & Bank Breakdown
        story.append(Paragraph("Schedule 2: Detailed Cash & Bank Calculation", self.styles['SectionHeader']))
        
        # Calculate cash balance step by step
        cash_detail_data = [
            ['Component', 'Amount (N)', 'Source', 'Notes']
        ]
        
        # Opening cash balance (use the same variable as main SOA)
        opening_cash_balance = cash_balance  # This comes from tax_data
        
        # Opening cash balance
        cash_detail_data.append([
            'Opening Cash Balance',
            format_currency(opening_cash_balance),
            'User Profile',
            'Set in Cash/Bank Management'
        ])
        ...
        # Subtract expenses (decreases cash)
        cash_detail_data.append([
            'Less: Total Expenses',
            format_currency(-total_expenses_detailed),
        ...
        # Subtract drawings (decreases cash)
        if drawings > 0:
            cash_detail_data.append([
                'Less: Owner Drawings',
                format_currency(-drawings),
                'User Profile',
                'Personal withdrawals'
            ])
        
        # Add capital contributions (increases cash)
        if capital > 0:
            cash_detail_data.append([
                'Add: Capital Contributions',
                format_currency(capital),
                'User Profile',
                'Owner investments'
            ])
        
        # Calculate theoretical cash balance
        theoretical_cash = opening_cash_balance + total_income_detailed - total_expenses_detailed - drawings + capital
        
        cash_detail_data.append(['', '', '', ''])  # Spacer
        cash_detail_data.append([
            'CALCULATED CASH BALANCE',
            format_currency(theoretical_cash),
            'Accounting Formula',
            'Opening + Income - Expenses - Drawings + Capital'
        ])
        
        # Show current cash balance from SOA
        cash_detail_data.append([
            'SOA REPORTED CASH BALANCE',
        ...
        # Show difference
        cash_difference = theoretical_cash - cash_balance
        cash_detail_data.append([
            'DIFFERENCE',
            format_currency(cash_difference),
            'Variance Analysis',
        ...
        # Schedule 3: Detailed Equity Breakdown
        story.append(Paragraph("Schedule 3: Detailed Owner's Equity Calculation", self.styles['SectionHeader']))
        
        equity_detail_data = [
            ['Component', 'Amount (N)', 'Source', 'Notes']
        ]
        
        equity_detail_data.append([
            'Opening Equity',
            format_currency(opening_equity),
            'User Profile',
        ...
        # Calculate theoretical closing equity
        theoretical_equity = opening_equity + net_profit_detailed + capital - drawings
        
        equity_detail_data.append(['', '', '', ''])  # Spacer
        equity_detail_data.append([
            'CALCULATED CLOSING EQUITY',
            format_currency(theoretical_equity),
            'Accounting Formula',
            'Opening + Profit + Capital - Drawings'
        ])
        
        equity_detail_data.append([
            'SOA REPORTED CLOSING EQUITY',
            format_currency(closing_equity),
            'SOA Report',
            'Equity shown in Balance Sheet'
        ])
        
        # Show difference
        equity_difference = theoretical_equity - closing_equity
        equity_detail_data.append([
            'DIFFERENCE',
            format_currency(equity_difference),
            'Variance Analysis',
            'Calculated vs Reported'
        ])
        ...
        # Schedule 4: Detailed Liability Breakdown
        story.append(Paragraph("Schedule 4: Detailed Liability Breakdown", self.styles['SectionHeader']))
        
        # Get detailed liability information
        liability_detail_data = [
            ['Liability Type', 'Amount (N)', 'Count', 'Status', 'Notes']
        ]
        
        # Accounts Payable (Creditors)
        liability_detail_data.append([
            'Accounts Payable (Creditors)',
            format_currency(creditors_value),
            f'{creditors_count} vendors',
            'Outstanding' if creditors_count > 0 else 'None',
        ...
        liability_detail_data.append([
            'Loans Payable',
            format_currency(loans_outstanding),
            f'{len(loans)} loans' if loans_outstanding > 0 else '0 loans',
            'Outstanding' if loans_outstanding > 0 else 'None',
        ...
        liability_detail_data.append([
            'Estimated Tax Payable',
            format_currency(unpaid_tax),
            'Tax calculation',
            'Estimated' if unpaid_tax > 0 else 'None',
            'Unpaid tax obligation'
        ])
        
        liability_detail_data.append(['', '', '', '', ''])  # Spacer
        liability_detail_data.append([
            'TOTAL CURRENT LIABILITIES',
            format_currency(total_current_liabilities),
            'Sum of all',
            'Outstanding',
            'All liability types combined'
        ])
        ...
        # Schedule 5: Accounting Equation Verification
        story.append(Paragraph("Schedule 5: Accounting Equation Verification", self.styles['SectionHeader']))
        
        verification_data = [
            ['Component', 'Calculated (N)', 'SOA Reported (N)', 'Difference (N)', 'Status']
        ]
        
        # Assets
        calculated_assets = total_assets_nbv + theoretical_cash + inventory_value + debtors_value
        verification_data.append([
            'Total Assets',
            format_currency(calculated_assets),
            format_currency(total_all_assets),
            format_currency(calculated_assets - total_all_assets),
            '✓' if abs(calculated_assets - total_all_assets) < 1 else '⚠'
        ])
        ...
        verification_data.append([
            ...
            format_currency(closing_equity),
            format_currency(equity_difference),
            '✓' if abs(equity_difference) < 1 else '⚠'
        ])
        
        # Total Liabilities + Equity
        calculated_liab_equity = total_current_liabilities + theoretical_equity
        reported_liab_equity = total_current_liabilities + closing_equity
        
        verification_data.append(['', '', '', '', ''])  # Spacer
        verification_data.append([
            'Liabilities + Equity',
            format_currency(calculated_liab_equity),
            format_currency(reported_liab_equity),
            format_currency(calculated_liab_equity - reported_liab_equity),
            '✓' if abs(calculated_liab_equity - reported_liab_equity) < 1 else '⚠'
        ])
        
        # Accounting equation balance
        equation_balance = calculated_assets - calculated_liab_equity
        verification_data.append([
            'EQUATION BALANCE',
            format_currency(equation_balance),
            format_currency(accounting_balance),
            format_currency(equation_balance - accounting_balance),
            '✓' if abs(equation_balance) < 1 else '⚠'
        ])
        ...
        story.append(Paragraph("Analysis & Recommendations", self.styles['SectionHeader']))
        
        analysis_text = f"""
<b>Key Findings:</b><br/>
<br/>
1. <b>Revenue Analysis:</b> Total Income = ₦{safe_float(total_income_detailed):,.2f}<br/>
   • If negative, check for liability adjustments miscategorized as income<br/>
   • Review sourceType patterns for unusual entries<br/>
<br/>
2. <b>Cash Balance Variance:</b> ₦{safe_float(cash_difference):,.2f}<br/>
   • Calculated: ₦{safe_float(theoretical_cash):,.2f}<br/>
   • Reported: ₦{safe_float(cash_balance):,.2f}<br/>
   • This may indicate missing transactions or incorrect opening balance<br/>
<br/>
3. <b>Equity Variance:</b> ₦{safe_float(equity_difference):,.2f}<br/>
   • This affects the accounting equation balance<br/>
   • Review capital contributions and drawings tracking<br/>
<br/>
4. <b>Accounting Equation Balance:</b> ₦{safe_float(equation_balance):,.2f}<br/>
   • Should be zero for perfect balance<br/>
   • Current difference: ₦{safe_float(accounting_balance):,.2f}<br/>
<br/>
<b>Immediate Actions Needed:</b><br/>
• Review transactions with negative amounts in income<br/>
• Verify opening cash balance is correctly set<br/>
• Check for missing capital contribution entries<br/>
• Investigate any liability adjustments affecting P&L<br/>
• Ensure all sourceTypes are correctly categorized
"""
...
def generate_balance_breakdown_pdf(user_data, balance_data):
    ...
    Generated by FiCore Africa - No Big Grammar Digital Finance<br/>
