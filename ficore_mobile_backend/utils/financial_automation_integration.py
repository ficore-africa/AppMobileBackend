...
def calculate_total_liabilities(mongo) -> Dict[str, Any]:
    ...
    FIXED: Now includes ALL liability types (FC, Subscription, Fee Waiver)
    """
    try:
        ...
        # Fee Waiver Liabilities (CRITICAL FIX - was missing!)
        ...
        fee_waiver_liability_total = sum(entry.get('amount', 0) for entry in fee_waiver_liabilities)
        total_liabilities = fc_liability_total + subscription_liability_total + fee_waiver_liability_total
        ...
        return {
            ...
            'fee_waiver_liabilities': {
                'count': len(fee_waiver_liabilities),
                'total': fee_waiver_liability_total
            },
            'total_liabilities': total_liabilities,
            'message': f'Total outstanding liabilities: ₦{safe_float(total_liabilities):,.2f} (FC: ₦{safe_float(fc_liability_total):,.2f}, Subscription: ₦{safe_float(subscription_liability_total):,.2f}, Fee Waiver: ₦{safe_float(fee_waiver_liability_total):,.2f})'
        }
